...
os.environ["LANGCHAIN_PROJECT"] = f"Azza Man - {unique_id}"
...
class State(MessagesState):
    """Represents the state of the conversation and user financial data."""
    username: str  
    income: float
...
def show_assistant_output(message: str):
    """Display the assistant's output to the console."""
    print(message)


# Initialize ChatGroq with the specified model
GPT = ChatGroq(model_name="llama-3.3-70b-versatile")

class QueryRouting(BaseModel):
    """Defines the routing for user queries to different registrars."""
...
def assistant(state: dict) -> str:
    ...
    # Define the query prompt for the assistant
    query_prompt = (
        """
        Azza Man is a personal financial assistant designed to help users manage their budget, log expenses,
        and provide expert financial commentary on their financial status. Analyze the user's query and respond based on the context:
          ...
          * 'intro': If the query is a greeting or general inquiry about Azza Man's role.
          * 'irrelevant': For any other type of query not related to Azza Man's responsibilities.
        """
    )
...
def intro(state: State) -> State:
    """
    Introduces Azza Man to the user and identifies their needs.

    Args:
        state (State): The current state of the application.

    Returns:
        State: Updated state after the introduction.
    """
    messages = state.get("messages")
    ...
    introduction_message = (
        f"""
        You are Azza Man, a personal financial assistant designed to help a user manage their budget, log expenses,
    ...
    # Capture user's response using interrupt
    user_response = interrupt("Please provide your response:")  # Use interrupt to capture user response
    messages.append(HumanMessage(content=user_response))

    return state

class QueryResponse(BaseModel):
    """Defines the response structure for query evaluation."""
...
def evaluator(state: dict) -> str:
    """
    Evaluates the user's response to the introduction.
...
class BudgetStateUpdate(BaseModel):
    """Defines the structure for updating budget state."""
    income: float
    savings_goal: float
    currency: str

# Create a structured output model for budget updates
...
def budget_registrar(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    budget_message = (
        f"""
          Azza Man is a personal financial assistant engaged in an ongoing conversation with the user. The user intends to
          ...
          Instructions:
            ...
            2. Request Missing Details:
               ...
               conversational, yet focused on Azza Man’s responsibilities.
            ...
            5. Stay Concise and Relevant:
               ...
               participation while staying aligned with Azza Man's responsibilities.
          """
    )
...
class BudgetRegistryEvaluation(BaseModel):
    """Defines the structure for evaluating budget registry responses."""
    next_node: Literal['budget_allocation_node', 'budget_registrar', 'END']

# Create a structured output model for budget evaluation
...
class ExpenseStateUpdate(BaseModel):
    """Defines the structure for updating expense state."""
    expenses: List[float]
    currency: str

# Create a structured output model for expense updates
...
def expense_registrar(state: State) -> State:
    ...
    # Display the assistant's response
    show_assistant_output(f"\033[92m{registrar_response}\033[0m")

    # Capture the user's response using interrupt
    user_response = interrupt("Please provide your response:")  # Use interrupt to capture user response
    messages.append(HumanMessage(content=user_response))
...
class ExpenseResponse(BaseModel):
    """Defines the structure for expense evaluation responses."""
    next_node: Literal['expense_logging_node', 'expense_registrar', 'END']

# Initialize the GPT model with structured output for expense evaluation
...
class ExpertStateUpdate(BaseModel):
    """Defines the structure for updating expert state."""
    income: float 
    expenses: float
    savings_goal: float
    currency: str

# Create a structured output model for expert updates
...
def expert_registrar(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Define the system message guiding the assistant's response
    expert_message = f"""
          Azza Man is a personal financial assistant engaged in an ongoing conversation with the user. 
          ...
          Instructions:
            ...
            2. Request Missing Details:
               ...
               Be dynamic and slightly conversational, yet focused on Azza Man’s responsibilities.
               Example: "Could you let me know your savings goal? This will help me tailor your budget effectively."
            3. Display Data Only When Requested:
               Only display the user’s data if explicitly requested, and ensure it is formatted correctly and user-friendly.
...
class ExpertResponse(BaseModel):
    """Defines the structure for expert evaluation responses."""
    next_node: Literal['ask_expert', 'expert_registrar', 'END']

# Configure GPT with Pydantic validation for expert evaluation
...
def expert_registry_eval(state: dict) -> str:
    ...
    prompt = (
        ...
        Azza Man's last message: {messages[-2].content}
        """
    )
...
class BudgetAllocation(BaseModel):
    """Defines the structure for budget allocation."""
    income: float
    savings_goal: Union[float, str]  # Savings goal can be a float, percentage, ratio, or amount

# Create a structured output model for budget allocation
...
class Existence(Enum):
    """Enumeration for checking the existence of a query."""
    EXIST = "EXIST"
    DOES_NOT_EXIST = "DOES NOT EXIST"

class MsgCheck(BaseModel):
    """Defines the structure for checking message existence."""
    query: Existence

# Create a structured output model for message checking
MSG_MODEL = GPT.with_structured_output(MsgCheck)

def ask_expert(state: State) -> State:
    """
    Prompts the user to ask a financial expert question.

    Args:
        state (State): The current state of the application.

    Returns:
        State: Updated state after prompting the user.
    """
    messages = state.get("messages")

    # Define the prompt for analyzing the user's query
    prompt = f"""
    Azza Man is a personal financial assistant engaged in an ongoing conversation with the user. The user appears interested in querying the financial expert.
    ...
    response = MSG_MODEL.invoke(prompt)
    ...
    if query_status == Existence.DOES_NOT_EXIST:
        ...
        show_assistant_output(f"\033[92m{assistant_prompt}\033[0m")
...
def financial_expert_advice(state: State) -> State:
    """
    Provides financial advice based on user input and current financial state.

    Args:
        state (State): The current state of the application.

    Returns:
        State: Updated state after providing financial advice.
    """
    messages = state.get("messages")
    user_response = messages[-1].content
    last_ai_message = messages[-2].content
    ...
    prompt = (
        f"""
        Azza Man is a personal financial assistant designed to help users manage their budget, log expenses, and provide expert
    ...
    messages.append(AIMessage(content=response))
    show_assistant_output(response)
...
class SummarizeConversation(BaseModel):
    """Defines the structure for summarizing the conversation."""
    summary: str
...
def summarize_conversation(state: State) -> State:
    ...
    # Create a summarization prompt if the message count exceeds a threshold
    if len(messages) > 6:
        if summary:
...
def Continue(state: State) -> State:
    ...
    Returns:
        State: Updated state after asking the user.
    """
    messages = state.get("messages")
    ...
    # Capture user's response using interrupt
    user_response = interrupt("Please provide your response:")  
    messages.append(HumanMessage(content=user_response))
    return state

class EvaluationResponse(BaseModel):
    """Defines the structure for evaluation responses."""
    next_node: Literal['budget_registrar', 'expense_registrar', 'expert_registrar', 'END', 'irrelevant']

# Create a structured output model for continue evaluation
CONTINUE_EVAL = GPT.with_structured_output(EvaluationResponse)

def continue_eval(state: dict) -> str:
    """
    Evaluates user's response to continue the session.

    Args:
        state (dict): The current application state, including messages.

    Returns:
        str: The determined next node based on the user's response.
    """
    messages = state.get("messages")
    ...
    prompt = (
        f"""
            Azza Man is a personal financial assistant designed to help users with the following tasks:
            ...
            Task:
              * Analyze the user's response and determine the appropriate next action based on its context:
                ...
                - Return 'irrelevant' for any query that falls **outside the scope** of Azza Man’s responsibilities.
...
def irrelevant(state: State) -> State:
    ...
    messages += [
        AIMessage(
            """
            I am Azza Man, your AI personal financial assistant. I can help you with the following:
...
class IrrelevantResponse(BaseModel):
    """Defines the structure for irrelevant response evaluation."""
    next_node: Literal['budget_registrar', 'expense_registrar', 'expert_registrar', 'END']

# Create a structured output model for irrelevant evaluation
...
def irrelevant_eval(state: dict) -> str:
    ...
    query_prompt = (
        ...
        User response: {user_response}
        Azza Man's last message: {last_ai_message}
        """
    )
...
def BudgetFirst(state: State) -> State:
    ...
    Returns:
        State: Updated state after checking for budget creation.
    """
    messages = state.get("messages")
    ...
    if not income:
        ...
        show_assistant_output(f"\033[92m{msg}\033[0m")
...
workflow.add_edge("financial_expert_advice", "Continue")  # Connect to continue node
...
azza_man = workflow.compile(checkpointer)

def invoke_our_graph(st_messages, callables):
    """
    Invokes the workflow graph with the current messages and callback configuration.

    Args:
        st_messages (list): The current state messages.
        callables (list): A list of callback functions.

    Returns:
        dict: The result of invoking the workflow, including messages.
    """
    ...
    result = azza_man.invoke({"messages": st_messages}, config={"callbacks": callables, "configurable": {"thread_id": "1"}})

    # Check if the result contains the messages
    if "messages" in result:
        return result  # Return the entire result, which should include messages
    else:
        raise ValueError("Invalid response format from the graph invocation.")